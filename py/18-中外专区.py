...
# -*- coding: utf-8 -*-
"""
中外专区 Spider - 四壳通用Python Spider
站点: https://cwp.zwzq9.lat/zwzq/
"""
...
class Spider(Spider):
    def init(self, extend=""):
        self.siteUrl = "https://cwp.zwzq9.lat/zwzq"
        self.rawUrl = "https://cwp.zwzq9.lat/zwzq"
        return "中外专区"
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        list = []
        items = re.findall(r'<a href="(/\d+\.html)"[^>]*>(.*?)</a>', html, re.S)
        seen = set()
        for href, item in items:
            id_m = re.search(r'/(\d+)\.html', href)
            if not id_m or id_m.group(1) in seen:
                continue
            # 只匹配视频卡片（包含img或background-image的）
            if '<img' not in item and 'background-image' not in item:
                continue
            seen.add(id_m.group(1))
            title_m = re.search(r'title="([^"]*)"', item) or re.search(r'<p class="title">(.*?)</p>', item) or re.search(r'<div class="title">(.*?)</div>', item)
            img_m = re.search(r'<img src="([^"]*)"', item) or re.search(r'background-image:\s*url\(([^)]*)\)', item)
            list.append({
                "vod_id": id_m.group(1),
    ...
    def detailContent(self, ids):
        ...
        vod_list = [{
            ...
            "vod_play_from": "中外专区", "vod_play_url": play_url
        }]
